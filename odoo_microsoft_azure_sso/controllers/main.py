        ...
        for provider in providers:
            ...
            if provider['microsoft_provider']:
                ...
                if provider.get("microsoft_tenant_id") != 'common':
                    auth_endpoint = provider['auth_endpoint'].replace('common', provider["microsoft_tenant_id"])
...
class OAuthController(http.Controller):
    ...
    def microsoft_signin(self, **kw):
        ...
        try:
            ...
            authorization_data = provider.sudo().get_microsoft_oauth_token(kw.get("code"), refresh_token=None)
            ...
            try:
                ...
                response = requests.get(provider.data_endpoint + "/v1.0/me", headers=headers)
            ...
            _, login, key = request.env['res.users'].with_user(SUPERUSER_ID).microsoft_auth_oauth(provider.id, kw)
