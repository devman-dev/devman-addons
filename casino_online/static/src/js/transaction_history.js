...
publicWidget.registry.TransactionHistory = publicWidget.Widget.extend({
  ...
  _loadTransactionTable() {
    const transactionBody = this.transactionTable.find(".o_transaction_list_content");
    const transactionFooter = this.transactionTable.find(".o_transaction_list_footer")
    ...
    if (Object.keys(this.transactionData).length === 0) {
      ...
      transactionFooter.find(".o_transaction_list_total").append(`${formatCurrency(0.0, this.currencyId)}`)
    } else {
      ...
      if (!transactionFooter.find(".o_transaction_list_total").is(":empty")) transactionFooter.find(".o_transaction_list_total").empty();
      transactionFooter.find(".o_transaction_list_total").append(`${formatCurrency(total_per_page, this.currencyId)}`)
    }
  },
  _loadTransactionPaginationInfo(){
    ...
    if (Object.keys(this.transactionData).length === 0) {
      transactionFooter.append("Mostrando 0 de 0 transacciones");
    } else {
      if (!transactionFooter.is(":empty")) transactionFooter.empty();

      let displayOffset = this.pageOffset + this.maxPageContent;
      const maxTransactions = Object.entries(Object.keys(this.transactionData)).at(-1)[1];
      if (this.pageOffset === parseInt(maxTransactions)) {
        displayOffset = (parseInt(maxTransactions) + 1);
      }
      const displayText = `Mostrando ${displayOffset} de ${parseInt(maxTransactions) + 1} transacciones`
  ...
  _loadTransactionPagination() {
    ...
    if (Object.keys(this.transactionData).length === 0) {
      ...
      let end_classes = ["page-item", this.pageOffset === parseInt(maxTransactions.at(-1)[1]) ? "disabled": ""];
      ...
      for (let i = 0; i < maxTransactions.length; i++) {
        let class_list = ["page-item", this.pageOffset === parseInt(maxTransactions[i][1]) ? "active": ""]
        transactionPagination.append(
          `<li class="${class_list.join(" ")}">
            <a class="page-link o_transaction_pagination_move step" data-offset-step="${parseInt(maxTransactions[i][1])}" href="#">${i + 1}</a>
          </li>`
        );
      }
      transactionPagination.append(
  ...
  async _loadTransactionData() {
    ...
    const paymentTransaction = await this.orm.searchRead(
      this.resModel,
      domain,
      fields
    );
