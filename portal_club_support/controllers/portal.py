...
Controladores del portal: registro público, selección de clubes, dashboard.
"""
...
class PortalClubSupport(CustomerPortal):
    ...
    def _handle_register_post(self, **kw):
        """Procesa el POST del formulario de registro público."""
        ...
        if not age_confirmed:
            return request.render("portal_club_support.jj_register_page", {
                ...
                "cpf": cpf,
                "birth_date": birth_date,
                "error": "Você precisa confirmar que tem mais de 18 anos.",
            })

        # ODOO18_PORTAL_FIX: validate the actual birth date server-side.
        try:
        ...
        if age < 18:
            return request.render("portal_club_support.jj_register_page", {
                "name": name, "email": email, "cpf": cpf,
                "birth_date": birth_date,
                "error": "O cadastro é permitido somente para maiores de 18 anos.",
            })
        ...
        try:
            # ODOO18_PORTAL_FIX: keep ORM failures inside a savepoint so an
            # error page can still be rendered with a valid transaction.
            ...
            # ODOO18_PORTAL_FIX: Odoo 18 uses a credential mapping.
            # authenticate() opens its own cursor, so the new account must be
            # committed first (same sequence used by auth_signup).
    ...
    def club_support_main(self, **kw):
        """Dashboard principal con resumen de clubes apoyados."""
        partner = request.env.user.partner_id
        ...
        for pref in preferences:
            club = pref.casino_club_id or pref.club_id
    ...
    def club_support_save(self, **kw):
        """Guarda preferencias de clubes. POST desde Step 2."""
        partner = request.env.user.partner_id
        ...
        # Procesar selecciones del formulario (club_<category_id> = <club_id>)
        saved = []
        for key, value in kw.items():
            ...
            # FEDERATION_CLUB_PICKER: validate against the federation catalogue.
            club = Club.browse(club_id)
            if not club.exists() or not club.active:
                continue

            # Buscar preferencia existente activa
            ...
            saved.append({"category": category.name, "club": club.name, "color": club.primary_color or "#0f3d2a"})
